...
#################################################################
#																#
#	@description: Script para realizar un start controlado de 	#
...
executeStartServer(){
    host=$1 && server=$2
    echo "»»» $server: WARNING starting..."
    executeCommand "/host=$host/server=$server:start(blocking=true)"
    executeCommand "/host=$host/server-config=$server:read-resource(include-runtime=true)"
    status=$(echo "$RESPONSE" | grep -oE '"status" => "\w*"')

    if [[ $(echo "$status" | grep -o "STOPPED") == "STOPPED" ]]; then
        echo "»»» $server: WARNING starting process..."
        for i in {1..4}; do
            executeCommand "/host=$host/server=$server:start(blocking=true)" true
            executeCommand "/host=$host/server-config=$server:read-resource(include-runtime=true)"
            status=$(echo "$RESPONSE" | grep -oE '"status" => "\w*"')
            ...
            if [[ $(echo "$status" | grep -o "STARTED") == "STARTED" ]]; then
                if [[ $i -eq 4 ]]; then
                    ...
                    echo "$server: ERROR executing starting process in server."
                    exit 99
                fi
            else
                break
...
echo "»»» WARNING: starting group $GROUP"
executeCommand "/server-group=$GROUP:start-servers(blocking=true)" true
...
for host in ${HOSTS}; do
    ...
    for server in ${SERVERS}; do
        ...
        if [[ "$group" == "$GROUP" ]]; then
            if [[ $(echo "$status" | grep -o "STOPPED") == "STOPPED" ]]; then
                executeStartServer "$host" "$server"
            else
                printInfoServer "$host" "$server" "$status"
            fi
        fi
